...
CONF_APP_HTML_INCLUDE = "app_html_include"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_APP_HTML_INCLUDE): cv.file_,
...
async def to_code(config):
    ...
    if CONF_APP_HTML_INCLUDE in config:
        cg.add_define("USE_WEBSERVER_APP_HTML_INCLUDE")
        path = CORE.relative_config_path(config[CONF_APP_HTML_INCLUDE])
        with open(file=path, encoding="utf-8") as app_file:
            add_resource_as_progmem("APP_HTML_INCLUDE", app_file.read())
