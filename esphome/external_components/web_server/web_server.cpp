...
// LittleFS will be enabled later - for now compile without it
// #ifdef USE_ESP32
// #include <LittleFS.h>
// #endif
...
// Temporarily disabled - LittleFS support will be added later
// #ifdef USE_ESP32
//   // Initialize LittleFS for static file serving
//   if (!LittleFS.begin(true)) {
//     ESP_LOGW(TAG, "LittleFS mount failed - static file serving will not be available");
//   } else {
//     ESP_LOGI(TAG, "LittleFS mounted successfully for static file serving");
//   }
// #endif
...
std::string WebServer::get_content_type_(const std::string &path) {
  ...
  if (path.find(".eot") != std::string::npos) return "application/vnd.ms-fontobject";
  return "text/plain";
}

#ifdef USE_WEBSERVER_APP_HTML_INCLUDE
void WebServer::handle_app_request(AsyncWebServerRequest *request) {
#ifndef USE_ESP8266
  AsyncWebServerResponse *response =
      request->beginResponse(200, "text/html", ESPHOME_WEBSERVER_APP_HTML_INCLUDE, ESPHOME_WEBSERVER_APP_HTML_INCLUDE_SIZE);
#else
  AsyncWebServerResponse *response =
...
void WebServer::handle_static_file_request(AsyncWebServerRequest *request) {
  ...
  for (size_t i = 0; i < STATIC_FILES_COUNT; i++) {
    if (url == STATIC_FILES[i].url) {
      ESP_LOGD(TAG, "Serving static file: %s", url.c_str());
#ifndef USE_ESP8266
      AsyncWebServerResponse *response =
          request->beginResponse(200, STATIC_FILES[i].content_type, STATIC_FILES[i].data, STATIC_FILES[i].size);
#else
      AsyncWebServerResponse *response =
  ...
  ESP_LOGW(TAG, "Static file not found: %s", request->url().c_str());
  request->send(404, "text/plain", "Not Found");

// #ifdef USE_ESP32
//   std::string url = request->url().c_str();
//   std::string file_path = "/www" + url;
//
//   ESP_LOGD(TAG, "Static file request for: %s", url.c_str());
//
...
//   }
//
//   ESP_LOGD(TAG, "Looking for file: %s", file_path.c_str());
//
//   // Check if file exists
//   if (LittleFS.exists(file_path.c_str())) {
//     std::string content_type = this->get_content_type_(file_path);
//     ESP_LOGD(TAG, "Serving file with type: %s", content_type.c_str());
//     request->send(LittleFS, file_path.c_str(), content_type.c_str());
//   } else {
//     // Try to serve index.html for SPA routing
//     std::string default_path = "/www/index.html";
//     if (LittleFS.exists(default_path.c_str())) {
//       ESP_LOGD(TAG, "Serving default file for SPA: %s", default_path.c_str());
//       request->send(LittleFS, default_path.c_str(), "text/html");
//     } else {
//       ESP_LOGW(TAG, "File not found: %s", file_path.c_str());
//       request->send(404, "text/plain", "File Not Found");
//     }
//   }
// #else
//   request->send(404, "text/plain", "Static files not supported on this platform");
// #endif
}
